const BACKEND_URL = 'https://scriible-backend.onrender.com'; // change only if you move the server
...
const PALETTE_NAMES = { "#000000":"Black","#e6194b":"Red","#3cb44b":"Green","#ffe119":"Yellow","#4363d8":"Blue","#f58231":"Orange","#911eb4":"Purple","#46f0f0":"Cyan","#f032e6":"Magenta","#bcf60c":"Lime","#fabebe":"Pink","#008080":"Teal","#e6beff":"Lavender":"#e6beff","#9a6324":"Brown","#fffac8":"Beige","#800000":"Maroon","#aaffc3":"Mint","#808000":"Olive","#ffd8b1":"Peach","#000075":"Navy","#808080":"Gray","#ffffff":"White"};
...
PALETTE_COLORS.forEach(c=>{
  ...
  s.onclick=()=>{ selectedColor=c; document.getElementById('colorPicker').value=c;
...
document.getElementById('clearBtn').onclick=()=>{ if(confirm('Clear canvas?')){ ctx.clearRect(0,0,LOGICAL_WIDTH,LOGICAL_HEIGHT); socket.emit('clearCanvas'); }};

// Socket events
socket.on('roomJoined',(code,players,maxRounds)=>{ currentMaxRounds=maxRounds; roomCode=code;
  document.getElementById('roomDisplay').textContent=code;
  document.getElementById('roundInfo').textContent=`Round 0/${maxRounds}`;
  startGame(); updatePlayers(players);
});
...
socket.on('yourTurn',choices=>{
  const d=document.getElementById('wordChoices'); d.innerHTML='';
  choices.forEach(w=>{ const b=document.createElement('button'); b.textContent=w;
...
function sendChat(){
  ...
  if(i.value.trim()){ socket.emit('chatMessage',i.value.trim()); i.value=''; }
}
...
function addMessage(u,t){
  ...
  d.innerHTML=`<strong>${u}:</strong> ${t}`;
  document.getElementById('messages').appendChild(d);
  d.scrollIntoView({behavior:'smooth'});
}
function updatePlayers(players){
  const c=document.getElementById('players');
  c.innerHTML='<h3 style="color:#FFA447;margin-bottom:8px;">Players</h3>';
  Object.values(players).sort((a,b)=>b.score-a.score).forEach(p=>{
    const d=document.createElement('div');
    d.className='player';
    d.textContent=`${p.name}${p.id===socket.id?' (you)':''} → ${p.score} pts`;
    c.appendChild(d);
  });
}